...
parser =
    ...
    <*> (
         (many $ strOption $
          short 'c' <>
          long "context" <>
          help "Pattern which must appear near <pattern>."))
    <*> (
         (many $ strOption $
          short 'C' <>
          long "not-context" <>
          help "Pattern which must not appear near <pattern>."))
    <*> (pure 0
         <|>
         (option auto $
          short 'B' <>
          long "before-context" <>
          help "Number of lines to search for context before match."))
    <*> (pure 0
         <|>
         (option auto $
          short 'A' <>
          long "after-context" <>
          help "Number of lines to search for context after match."))
